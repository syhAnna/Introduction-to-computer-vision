...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        if img is None:
            ...
            success = True
            self._images[key] = img.astype(np.uint8)
        #########################################
        return success, msg
...
success, errorMessage = exampleBasedInpainting(self)
        ...
        if (source is None) or (alpha is None):
            ...
            # source and alpha must have the same dimesions
            if source[:, :, 0].shape != alpha.shape:
                msg = 'Inconsistent image shape'
                return success, msg
            else:
                # source much be a 3-channel uint8 image
                flag = 0
                if len(source.shape) != 3:
                    msg = 'Source is not 3-channel'
                    flag = 1
                else:
                    if source.dtype != np.uint8:
                        msg = 'Source is 3-channel but not uint8 image'
                        flag = 1

                # alpha must be a one-channel uint8 image
                if len(alpha.shape) != 2:
                    if flag == 0:
                        msg = 'Alpha is not 1-channel'
                        flag = 1
                    else:
                        msg = ''.join([msg, ' and Alpha is not 1-channel'])
                else:
                    if source.dtype != np.uint8:
                        if flag == 0:
                            msg = 'Alpha is 1-channel but not uint8 image'
                            flag = 1
                        else:
                            msg = ''.join([msg, ' and Alpha is 1-channel but not uint8 image'])
